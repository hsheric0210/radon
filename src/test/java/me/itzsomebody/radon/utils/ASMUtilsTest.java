...
public class ASMUtilsTest
{
	...
	{
		Assert.assertFalse(ASMUtils.isInstruction(PowerMockito.mock(FrameNode.class)));
		Assert.assertFalse(ASMUtils.isInstruction(PowerMockito.mock(LabelNode.class)));
		Assert.assertFalse(ASMUtils.isInstruction(PowerMockito.mock(LineNumberNode.class)));
		Assert.assertTrue(ASMUtils.isInstruction(PowerMockito.mock(InsnNode.class)));
	}
	...
	{
		Assert.assertTrue(ASMUtils.isFloatInsn(new LdcInsnNode((float) Opcodes.NOP)));
	...
	{
		Assert.assertTrue(ASMUtils.isDoubleInsn(new LdcInsnNode((double) Opcodes.NOP)));
	...
	{
		Assert.assertEquals(Opcodes.ACONST_NULL, ASMUtils.getIntegerFromInsn(new InsnNode(Opcodes.ICONST_1)));

		Assert.assertEquals(Opcodes.ACONST_NULL, ASMUtils.getIntegerFromInsn(new IntInsnNode(Opcodes.ICONST_1, 1)));

		Assert.assertEquals(Opcodes.ACONST_NULL, ASMUtils.getIntegerFromInsn(new LdcInsnNode(Opcodes.ACONST_NULL)));
	...
	{
		Assert.assertEquals(Opcodes.NOP, ASMUtils.getLongFromInsn(new InsnNode(Opcodes.LCONST_0)));

		Assert.assertEquals(Opcodes.ACONST_NULL, ASMUtils.getLongFromInsn(new LdcInsnNode(1L)));
	...
	{
		Assert.assertEquals((float) Opcodes.NOP, ASMUtils.getFloatFromInsn(new InsnNode(Opcodes.FCONST_0)), 0);

		Assert.assertEquals((float) Opcodes.ACONST_NULL, ASMUtils.getFloatFromInsn(new LdcInsnNode(1.0f)), 0);
	...
	{
		Assert.assertEquals(Opcodes.NOP, ASMUtils.getDoubleFromInsn(new InsnNode(Opcodes.DCONST_0)), 0);

		Assert.assertEquals(Opcodes.ACONST_NULL, ASMUtils.getDoubleFromInsn(new LdcInsnNode(1.0)), 0);
	...
	{
		Assert.assertEquals("(ILjava/lang/Object;)[I", ASMUtils.getGenericMethodDesc("(ILjava/lang/Integer;)[I"));
	}
}
